...
class Install:
	...
	def set_installtion_path(self) -> None:
		...
		Returns:
			None
		"""
		self.install_path = input("Enter the installation path: ") 
	...
	def compile_py_file(self,file_name: str, post_fix: str="", exe_name:str=None) -> None:
		...
		dist_exe = os.path.join(self.install_path, "dist", os.path.splitext(file_name)[0] + ".exe")
		print(dist_exe)
...
if __name__ == "__main__":
	...
	print("#" * helpers.get_terminal_width())
	output = []
	with open(f"{ins.script_path}\\source\\cfiles\\singelCall.c", "r") as file:
		for line in file.read().split("\n"):
			if "*installPath" in line:
				output.append(f'    char *installPath = "{ins.bin_folder.replace("\\","\\\\")}\\\\{ins.program_name}-gate.exe";')
			else:
				output.append(line)
	with open(f"{ins.script_path}\\source\\cfiles\\singelCall1.c", "w") as file:
		file.write("\n".join(output))
	print("#" * helpers.get_terminal_width())
	print("Preparing EXE files")
	print("#" * helpers.get_terminal_width())
	...
	build_folder = os.path.join(ins.install_path, "build")
	dist_folder = os.path.join(ins.install_path, "dist")

	spec_file_1 = os.path.join(ins.install_path, os.path.splitext("guess.py")[0] + ".spec")
	spec_file_2 = os.path.join(ins.install_path, os.path.splitext("set_model.py")[0] + ".spec")
